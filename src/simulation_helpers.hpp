...
const int N = 10; // The number of images which construct a time series for each pixel
...
{   
	...
	printf("BOLD_signal size is %d x %d\n", (int)BOLD_signal.size(), (int)BOLD_signal[0].size());
	printf("filteredSignal size is %d x %d\n", (int)filteredSignal.size(), (int)filteredSignal[0].size());
...
#if 0 // Should return value othervice it is error in runtime
